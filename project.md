...
## Recent Changes
- **Dynamic Grid Scaling (Difficulty Balance):** Implemented a responsive formation system that equalizes difficulty between landscape and portrait. It uses a base "Target Threat" (independent of ratio) and calculates a 1.0x to 1.5x scale factor for invaders. The grid dynamically adjusts its columns to maintain ~60% horizontal coverage on any screen, ensuring consistent wall-bouncing and "Step Down" rates. All magic numbers migrated to `constants.js`. **[BALANCE]**
