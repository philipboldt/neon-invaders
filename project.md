...
- **Adaptive Grid Height Safety:** Refactored the `EntityManager` to intelligently calculate invader formation. If the grid becomes too tall (exceeding 45% of screen height) due to the new square 42x42 sprites, the system automatically compresses the formation by adding columns. Reduced the starting Y-ratio to 12% to move the formation up, reclaiming vertical play space. **[BALANCE/FIX]**
- **Invader Square Resizing:** Updated `CONSTANTS` to use 42x42 square dimensions for standard invaders. This eliminates visual distortion from the previous 1.5:1 rectangular ratio, ensuring the Gemini sprites appear at their intended aspect ratio with high fidelity. **[VISUAL]**
