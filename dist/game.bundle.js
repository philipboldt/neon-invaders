(() => {
  ...
  var UIManager = class {
    ...
    drawHUD(ctx2, gameState) {
      ...
      this.drawStat(ctx2, "Score: ", gameState.score, padding, y, "#39ff14");
      this.drawStat(ctx2, "Level: ", gameState.level, padding + colWidth, y, "#ff6600");
      this.drawStat(ctx2, "Lives: ", gameState.lives, padding + colWidth * 2, y, "#ff3366");
      const shieldText = gameState.shieldHits > 0 ? "ON" : gameState.hasShieldSystem ? "OFF" : "NONE";
      this.drawStat(ctx2, "Shield: ", shieldText, padding + colWidth * 3, y, "#00f5ff");
      const y2 = y + 25;
      this.drawStat(ctx2, "Pierce: ", gameState.hasPierce ? "YES" : "NONE", padding, y2, "#ffff00");
      this.drawStat(ctx2, "Damage: ", gameState.playerDamage, padding + colWidth, y2, "#39ff14");
      this.drawStat(ctx2, "Rocket: ", gameState.rocketLevel > 0 ? gameState.rocketLevel : "NONE", padding + colWidth * 2, y2, "#ff6600");
      ctx2.restore();
    }
    drawStat(ctx2, label, value, x, y, valueColor) {
      ctx2.fillStyle = "#00f5ff";
      ctx2.textAlign = "left";
      ctx2.fillText(label, x, y);
      const labelWidth = ctx2.measureText(label).width;
      ctx2.fillStyle = valueColor;
      ctx2.shadowColor = valueColor;
      ctx2.shadowBlur = 8;
      ctx2.fillText(value, x + labelWidth, y);
      ctx2.shadowBlur = 0;
    }
  ...
  var ParticleSystem = class {
    ...
    spawnStunningExplosion(cx, cy, color) {
      const layers = [
        ...
        // Ultra-fast core flash
        { count: 256, speed: 25, life: 150, size: 35, color },
        // Massive main burst
        { count: 256, speed: 12, life: 200, size: 20, color }
        // Screen-covering embers
      ];
      layers.forEach((layer) => {
        for (let n = 0; n < layer.count; n++) {
          ...
          p.maxSize = maxSize;
          p.life = 0;
          p.maxLife = layer.life * (0.8 + Math.random() * 0.4);
          p.color = layer.color;
          this.activeIndices.push(idx);
        }
      });
